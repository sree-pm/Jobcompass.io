...
export default function Jobs(){
  usePageMeta("30 live UK roles — JobCompass", "Fresh every morning. Every employer checked.", "/jobs");
  ...
  return (
    <MarketingLayout>
      <section style={{ maxWidth: 1100, margin:"0 auto", padding:"40px 24px" }}>
        <h1 style={{ fontSize:28, fontWeight:800 }}>Job library</h1>
        <p style={{ color:T.muted }}>Fresh every morning. Every employer checked.</p>
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search jobs (e.g. 'data', 'Manchester')" style={{ width:"100%", maxWidth:420, marginTop:12, padding:"10px 12px", border:`1px solid ${T.border}`, borderRadius:8 }} />
        {loading? <div style={{ marginTop:16, color:T.muted }}>Loading jobs…</div> : (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:12, marginTop:16 }}>
            ...
            {!jobs.length && <div style={{ color:T.muted, fontSize:12, background:T.surface, border:`1px dashed ${T.border}`, borderRadius:10, padding:16, textAlign:"center" }}>No matches. Try a broader search — new jobs land every morning.</div>}
          </div>
        )}
      </section>
    </MarketingLayout>
